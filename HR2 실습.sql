...
--1. 덧셈 연산자를 사용하여 모든 사원에 대해서 $300의 급여 인상을 계산한 후 사원이름, 급여, 인상된 급여를 출력하세요.
...
select ename 사원이름, salary 급여, salary * 12 + nvl(commission, +100 ) "연간 총 수입"  
from employee
order by salary * 12 + nvl(commission, +100 )desc ;
...
select ename 사원명, salary 급여, nvl(commission, 0 ) 보너스
from employee
where commission != 0
